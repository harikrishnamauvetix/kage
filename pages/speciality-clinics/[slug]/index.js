          ...
          {content?.overview?.symptoms?.heading &&
            content?.overview?.symptoms?.list?.length > 0 && (
              <>
                ...
                <Grid container spacing={2}>
                  {content?.overview?.symptoms.list.map((symptom, index) => (
          ...
          {content?.overview?.why_choose?.heading &&
            content?.overview?.why_choose?.steps?.length > 0 && (
              <Grid item xs={12}>
                ...
                <Grid container spacing={2}>
                  {content?.overview?.why_choose.steps.map((step, index) => (
                    <Grid item xs={12} sm={6} md={4} key={index}>
                      <Card>
                        {step.icon && (
                          <CardMedia
                            component="img"
                            height="140"
                            image={step.icon} // Assuming step.icon is the URL of the image
                            alt={step.title}
                          />
                        )}
                        <CardContent>
          ...
          <>
            ...
            <Grid container spacing={2} sx={{ marginTop: 2 }}>
              {service?.subpages?.map((subpage) => (
                <Grid item xs={12} sm={6} md={4} key={subpage?.page}>
                  <Card>
                    {subpage?.image && (
                      <CardMedia
                        component="img"
                        height="140"
                        image={subpage?.image} // Assuming subpage.image is the URL of the image
                        alt={subpage?.title}
                      />
                    )}
                    <CardContent>
                      <Link
                        href={`/speciality-clinics/${slug.replace(
                          / /g,
                          "-"
